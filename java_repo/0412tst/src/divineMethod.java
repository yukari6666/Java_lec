...
//		Double.parseDouble(String.format("%.2f",per2));
	...
	public void stringDivine() {
		...
		tmp = scan2.nextLine();
		try {
			this.importNumber1 =Double.parseDouble(tmp);
		} catch (Exception e) {
		...
		tmp = scan2.nextLine();
		try {
			this.importNumber2 = Double.parseDouble(tmp);
		} catch (Exception e) {
		...
		System.out.println("두 숫자의 나눈 값은 "+String.format("%.2f", this.divineNumber)+" 입니다.");
	}
}
